...
HashTable<K,V,Prober,Hash,KEqual>::HashTable(
    double resizeAlpha, const Prober& prober, const Hasher& hash, const KEqual& kequal)
       :  hash_(hash), kequal_(kequal), prober_(prober),resize_(resizeAlpha), size_(0), totalProbes_(0), mIndex_(0), occupied_(0)
{
...
{
    double loadfac = static_cast<double>(occupied_) / table_.size();
    if(loadfac>=resize_){
        resize();
    }
    ...
    if(table_[location] != NULL && !table_[location]->deleted){
...
void HashTable<K,V,Prober,Hash,KEqual>::remove(const KeyType& key)
{
    ...
    if(location == npos || table_[location] == NULL){
        return;
    }
    if(!table_[location]->deleted){
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    if(mIndex_ + 1 >= sizeof(CAPACITIES) / sizeof(HASH_INDEX_T)){
        throw std::logic_error("No more capcity");
    }
